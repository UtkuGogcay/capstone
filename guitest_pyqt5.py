...
class CameraApp(QMainWindow):
    ...
    def update_frame(self):
        ...
        if ret:
            ...
            # Draw the draggable points on the frame
            for point in self.points:
                frame_rgb = cv2.circle(frame_rgb, (point.x(), point.y()), 10, (0, 255, 0), -1)  # Green circle

            # Draw the polygon connecting the points
            points_np = np.array([[(point.x(), point.y()) for point in self.points]], np.int32)
            frame_rgb = cv2.polylines(frame_rgb, points_np, isClosed=True, color=(0, 0, 255), thickness=2)  # Red polygon
    ...
    def mousePressEvent(self, event):
        ...
        for i, point in enumerate(self.points):
            if (event.pos() - point).manhattanLength() <= 10:  # 10 is the radius of the circle
